<template>
  <div class="talent post-talent">
    ...
    <div class="talent-footer flex-row" v-if="enableToolbar">
      <f7-button big raised color="green" fill @click="applicationTalent">{{$t('talent.application')}}
        <span class="text" v-text="data.application_count ? '(' + data.application_count + ')' : ''"></span>
      </f7-button>
      <f7-button big raised color="green" fill @click="likeTalent">{{$t('talent.like')}}
        <span class="text" v-text="data.like_count ? '(' + data.like_count + ')' : ''"></span>
      </f7-button>
    </div>
    <div class="talent-footer flex-row" v-if="enableToolbar">
      <f7-button v-if="isOwner" big raised color="green" fill @click="updateTalent">{{$t('talent.update')}}</f7-button>
      <f7-button v-if="isOwner" big raised color="green" fill @click="deleteTalent">{{$t('talent.delete')}}</f7-button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    contentClick(data) {
      this.$emit('talent:content-click', data)
    },
