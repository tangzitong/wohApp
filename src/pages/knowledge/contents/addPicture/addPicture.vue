<template>
  <f7-page class="knowledge">
    <f7-navbar>
      ...
      <f7-nav-title :title="!knowledgecontentkey ? $t('knowledge.addpicture') : $t('knowledge.updatepicture')"></f7-nav-title>
    </f7-navbar>
    <f7-block>
      <h3>{{!knowledgecontentkey ? $t('knowledge.addpicture') : $t('knowledge.updatepicture')}}</h3>
    ...
    <f7-block v-if="isUserLogin">
      <f7-button big color="blue" style = "line-height:27px" @click="updateKnowledgePicture">{{!knowledgecontentkey ? $t('knowledge.addpicture') : $t('knowledge.updatepicture')}}</f7-button>
    </f7-block>
      ...
      if (this.knowledgecontentkey) {
        for (const knowledgecontent in this.knowledgecontents) {
          if (this.knowledgecontents[knowledgecontent].id === this.knowledgecontentkey) {
            ...
            const picturePath = this.knowledgecontents[knowledgecontent].content.picturePath
            window.store.dispatch('setImagePath', picturePath)
            break
          }
        }
      }
