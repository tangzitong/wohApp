<template>
  <f7-page class="knowledge-page">
    <f7-navbar :title="$t('knowledge.knowledge')" :back-link="$t('app.back')" sliding>
    </f7-navbar>
    <card :enableToolbar="false" :data="knowledge"></card>
    <div class="comments">
      <div class="title">
    ...
    <f7-toolbar class="custom-toolbar flex-row" bottom-md>
      <f7-link class="tool tool-border flex-rest-width" @click="openCommentPopup">
        ...
        <span class="text" v-text="knowledge.comment_count ? knowledge.comment_count : $t('home.comment')"></span>
      </f7-link>
...
export default {
  ...
  mounted() {
    ...
    this.knowledge = find(this.timeline, p => p.id === query.mid)
    this.getComments()
  },
  methods: {
    ...mapActions([
