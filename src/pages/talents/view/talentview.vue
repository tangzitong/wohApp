<template>
  <f7-page class="talent-page">
    <f7-navbar :title="$t('talent.talent')" :back-link="$t('app.back')" sliding>
    </f7-navbar>
    <card :enableToolbar="false" :data="talent"></card>
    <div class="comments">
      <div class="title">
    ...
    <f7-toolbar class="custom-toolbar flex-row" bottom-md>
      <f7-link class="tool tool-border flex-rest-width" @click="openCommentPopup">
        ...
        <span class="text" v-text="talent.comment_count ? talent.comment_count : $t('home.comment')"></span>
      </f7-link>
      <f7-link class="tool flex-rest-width" :class="{liked: talent.liked}" @click="toggleLike(talent.id, talent.liked)">
...
export default {
  ...
  mounted() {
    ...
    this.talent = find(this.timeline, p => p.id === query.mid)
    this.getComments()
  },
  methods: {
    ...mapActions([
