<template>
  <f7-page>
    ...
    <f7-toolbar tabbar :labels="!isAndroid">
      <f7-link :icon="!isAndroid ? 'iconfont icon-ios7homeoutline' : ''" :text="$t('app.talentfinder')" tab-link="#home" tab-link-active></f7-link>
      ...
      <f7-link :icon="!isAndroid ? 'iconfont icon-ios7person' : ''" :text="$t('app.talent')" tab-link="#data"></f7-link>
...
export default {
  ...
  methods: {
    ...
    tabActived(tab) {
      ...
      switch (this.activedTab) {
        case 'home':
          this.getApplicationTalents()
          break
        case 'contacts':
          this.getRoomList()
          break
        case 'data':
          this.getOwnerTalents()
          break
      }
    },
