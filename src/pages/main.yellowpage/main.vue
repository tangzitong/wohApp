<template>
  <f7-page>
    ...
    <f7-toolbar tabbar :labels="!isAndroid">
      <f7-link :icon="!isAndroid ? 'iconfont icon-ios7homeoutline' : ''" :text="$t('app.companyworker')" tab-link="#home" tab-link-active></f7-link>
      ...
      <f7-link :icon="!isAndroid ? 'iconfont icon-ios7person' : ''" :text="$t('app.companymanager')" tab-link="#data"></f7-link>
...
export default {
  ...
  computed: {
    ...
    navbarTitle() {
      switch (this.activedTab) {
        case 'home':
          return this.$t('app.app_company')
        case 'contacts':
          return this.$t('app.chat')
        case 'news':
          return this.$t('app.news')
        case 'data':
          return this.$t('app.companymanager')
        case 'settings':
  ...
  methods: {
    ...
    tabActived(tab) {
      ...
      switch (this.activedTab) {
        case 'home':
          this.getApplicationCompanys()
          break
        case 'contacts':
          this.getRoomList()
          break
        case 'data':
          this.getOwnerCompanys()
          break
      }
    },
    getApplicationCompanys() {
      this.$f7.preloader.show()
      this.$root.chat.getCompanyListByApplication(function(applicationcompanys) {
        window.store.dispatch('initApplicationCompanys', applicationcompanys)
      })
