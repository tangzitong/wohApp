<template>
  <f7-page class="event-page">
    <f7-navbar :title="$t('event.event')" :back-link="$t('app.back')" sliding>
    </f7-navbar>
    <card :enableToolbar="false" :data="event"></card>
    <div class="comments">
      <div class="title">
    ...
    <f7-toolbar class="custom-toolbar flex-row" bottom-md>
      <f7-link class="tool tool-border flex-rest-width" @click="openCommentPopup">
        ...
        <span class="text" v-text="event.comment_count ? event.comment_count : $t('home.comment')"></span>
      </f7-link>
      <f7-link class="tool flex-rest-width" :class="{liked: event.liked}" @click="toggleLike(event.id, event.liked)">
...
export default {
  ...
  mounted() {
    ...
    this.event = find(this.timeline, p => p.id === query.mid)
    this.getComments()
  },
  methods: {
    ...mapActions([
