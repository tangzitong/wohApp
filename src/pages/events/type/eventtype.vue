<template>
  <f7-page class="eventtype-page">
    <f7-navbar :title="$t('app.eventtype')" :back-link="$t('app.back')">
      <f7-nav-right>
        <f7-link :text="$t('app.display')" @click="viewEventtype"></f7-link>
        <f7-link v-if="isOwner === 'true'" :text="$t('app.add')" @click="addEventtype"></f7-link>
      </f7-nav-right>
    </f7-navbar>
...
export default {
  ...
  methods: {
    viewEventtype() {
      ...
      this.$f7router.navigate(`/event/?eventtype=${eventtype}&isowner=${this.isOwner}`)
    },
    addEventtype() {
      const eventtype = this.$$('input[name="eventtype-radio"]:checked').val()
      setEventtypeConfig(eventtype)
