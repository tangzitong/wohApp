<template>
  <f7-page>
    ...
    <f7-toolbar tabbar :labels="!isAndroid">
      <f7-link :icon="!isAndroid ? 'iconfont icon-ios7homeoutline' : ''" :text="$t('app.projectfinder')" tab-link="#home" tab-link-active></f7-link>
      ...
      <f7-link :icon="!isAndroid ? 'iconfont icon-ios7person' : ''" :text="$t('app.projectprovider')" tab-link="#data"></f7-link>
...
export default {
  ...
  computed: {
    ...
    navbarTitle() {
      switch (this.activedTab) {
        case 'home':
          return this.$t('app.app_project')
        case 'contacts':
          return this.$t('app.chat')
        case 'news':
          return this.$t('app.news')
        case 'data':
          return this.$t('app.projectprovider')
        case 'settings':
  ...
  methods: {
    ...
    getApplicationProjects() {
      ...
      this.$root.chat.getProjectListByApplication(function(applicationprojects) {
        window.store.dispatch('initApplicationProjects', applicationprojects)
      })
